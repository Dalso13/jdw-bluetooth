 ...
 * @property writeCharUuid 값을 쓸(Write) 특성 UUID (없으면 null)
 * @property notifyCharUuid 값을 받을(Notify) 특성 UUID
 * @property scanTimeoutMillis 스캔을 몇 초 동안 할지
 * @property isDebugMode 로그를 킬지 말지 (디버깅용)
 * @property scanMode 스캔 모드 설정 (Android ScanSettings 참고)
 */
interface BleConfig {
